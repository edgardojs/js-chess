...
function Quiescence(alpha, beta) {
	...
	for(MoveNum = GameBoard.moveListStart[GameBoard.ply]; MoveNum < GameBoard.moveListStart[GameBoard.ply + 1]; ++MoveNum) {
		...
		if(Score > alpha) {
            ...
            if(Move & MFLAGCAP == 0) {
               GameBoard.searchHistory[GameBoard.pieces[FROMSQ(Move)]  * BRD_SQ_NUM + TOSQ(Move)] += depth ** 2; 
            }
			alpha = Score;
			BestMove = Move;
		}		
...
function AlphaBeta(alpha, beta, depth) {
    ...
    // Search for best line by storing PV
    // GetPVMove() then orderPVMove()
...
function SearchPosition() {
    ...
    for(currentDepth = 1; currentDepth <= SearchController.depth; ++currentDepth) {
        ...
        line = "D: " + currentDepth + " Best Move: " + PrMove(BestMove) + " Score: " + bestScore +
